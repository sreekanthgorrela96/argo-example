...
After this, a Jenkins build that pushes a new tag and commits the updated `image:` in `k8s-manifests/base/deployment.yaml` still triggers Argo CD to sync and roll out on the on-prem cluster automatically.
